...
do
    ...
    do
	    ...
	    TEST_STRING2=$(grep 'Expected 50.0%:' $FILE_NAME)
	    if [ ${#TEST_STRING2} -gt 0 ]
	    then
		sed -i "/Test/ a\
hExpLimMid->SetBinContent(hExpLimMid->GetXaxis()->FindBin(${i}), hExpLimMid->GetYaxis()->FindBin(${j}), ${TEST_STRING2:20:7});" ExpLimMid.C
	    fi
        fi
    done
done
