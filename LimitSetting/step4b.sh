...
do
    ...
    do
	    ...
	    TEST_STRING2=$(grep 'Expected 84.0%:' $FILE_NAME)
	    if [ ${#TEST_STRING2} -gt 0 ]
	    then
		sed -i "/Test/ a\
hExpLimUp->SetBinContent(hExpLimUp->GetXaxis()->FindBin(${i}), hExpLimUp->GetYaxis()->FindBin(${j}), ${TEST_STRING2:20:7});" ExpLimUp.C
	    fi
        fi
    done
done
